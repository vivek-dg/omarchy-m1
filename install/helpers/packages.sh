...
omarchy_try_install_with_manager() {
	...
	yay)
		...
		# Special case for Hyprland on ARM64
		if [[ "$package" =~ ^hyprland(-git)?$ && "$(uname -m)" == "aarch64" ]]; then
			echo "[Omarchy] Building Hyprland with --nocheck for ARM64..."
			yay -S --noconfirm --needed --mflags "--nocheck" "$package"
		else
	...
	paru)
		...
		if [[ "$package" =~ ^hyprland(-git)?$ && "$(uname -m)" == "aarch64" ]]; then
			echo "[Omarchy] Building Hyprland with --nocheck for ARM64..."
			paru -S --noconfirm --needed --mflags "--nocheck" "$package"
		else
